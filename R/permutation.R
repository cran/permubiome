...
#Discardig features with minimal variation between classes
...
#Evaluating classes and splitting samples into groups
...
colnames(pvalue_matrix)<-c("Category", paste("Median " , classes[1], sep="") , paste("Median ", classes[2], sep=""), "p.value", "p.adj (fdr)")
...
if (ref_score >= 0){pvalue.i <- pnorm(ref_score, lower.tail=F)
...
if (pvalue.i != 0) {pvalue_matrix[(i-2),4] <- format((pvalue.i*2), digits = 7, scientific=F)
} else {pvalue_matrix[(i-2),2] <- 1.000
}
pb = txtProgressBar(min = 0, max = (ncol(df_norm) - 2), initial = 0, style = 3)
setTxtProgressBar(pb,(i-2))
invisible()
}
pvalue_matrix<-pvalue_matrix[order(pvalue_matrix[,4]),]
pvalue_matrix[,5] <- format(p.adjust(as.numeric(pvalue_matrix[,4], n = nrow(pvalue_matrix) , method = "fdr")), digits = 7, scientific=F)
cat("\n")
...
write.table(significant, file="permutation.output", quote=F, row.names=F, sep="\t")
...
save(df, df_norm, REFERENCE, classes, selected_features, nperm, tags.in, tags.out, pvalue_matrix, file="permubiome.RData")
}
